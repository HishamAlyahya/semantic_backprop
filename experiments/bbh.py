...
if __name__ == "__main__":
    ...
    cat = config["bbh_cat"] * 9 + array_id
    config["category"] = categories[cat % len(categories)]
    print(config)
    if os.environ.get("SLURM_JOB_ID") is not None:
        wandb.init(
            project="LLMBP_one_variant",
            config=config,
            tags=["bbh, 4_iters", "20 samples"],
        )

    train_data = get_split("train", category=config["category"])
    test_data = get_split("test", category=config["category"])
